...
import { ApiBadRequestResponse, ApiBearerAuth, ApiConflictResponse, ApiHeader, ApiNotFoundResponse, ApiOkResponse, ApiOperation, ApiTags, ApiUnauthorizedResponse } from '@nestjs/swagger';
...
export class StripeController {
  ...
  @ApiOkResponse({ schema: { example: { url: 'https://checkout.stripe.com/c/pay/cs_test_...' } } })
  @ApiUnauthorizedResponse({ description: 'Missing or invalid access token.' })
  @ApiBadRequestResponse({ description: 'Stripe could not create a checkout URL.' })
  ...
  @ApiOkResponse({ schema: { example: { url: 'https://billing.stripe.com/p/session/...' } } })
  ...
  @ApiOkResponse({ schema: { example: { success: true, cancelAtPeriodEnd: true } } })
  @ApiNotFoundResponse({ description: 'Subscription not found.' })
  ...
  @ApiOkResponse({ description: 'Returns the subscription or null.', schema: { nullable: true, example: { id: 'cm123subscription', status: 'ACTIVE', priceId: 'price_123', cancelAtPeriodEnd: false } } })
  ...
  @Post('webhook')
  @HttpCode(HttpStatus.OK)
  @ApiHeader({ name: 'stripe-signature', required: true, description: 'Signature generated by Stripe for the raw request body.' })
  @ApiOperation({ summary: 'Receive Stripe webhook events', description: 'Stripe-only endpoint. Requires the stripe-signature header and raw request body.' })
  @ApiOkResponse({ schema: { example: { received: true } } })
  @ApiBadRequestResponse({ description: 'Signature, raw body, or event metadata is invalid.' })
  @ApiConflictResponse({ description: 'A duplicate event is still being processed.' })
  async webhook(
